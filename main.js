...
//     if (VALIDATE(teamName) || !isNaN(teamName) || VALIDATE(driverName) || !isNaN(driverName) || VALIDATE(yearOfBirth) || isNaN(yearOfBirth) || VALIDATE(carBrand) || !isNaN(carBrand) || VALIDATE(sponsor) || VALIDATE(isAllowed)) {
...
const CHANGE_USER_DATA = (obj, field, value) => {
    if (VALIDATE(obj) || VALIDATE(field) || VALIDATE(value)) {
        return false;
    }
    return obj.field = value;
}
CHANGE_USER_DATA(USER_1, "city", "Kherson")
...
//     const RANDOM_SELLER = Math.floor(Math.random() * (Object.keys(obj).length));
